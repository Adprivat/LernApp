...
- **Global scope**: `C:/DEV/LernAPP/LernApp/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/DEV/LernAPP/LernApp/.claude/notes/`)
...
2. Glob `C:/DEV/LernAPP/LernApp/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/DEV/LernAPP/LernApp/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/DEV/LernAPP/LernApp/.claude/notes/` — works in any directory
